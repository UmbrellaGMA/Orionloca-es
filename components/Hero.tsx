...
const Hero: React.FC = () => {
  ...
  const safeHeadline = DOMPurify.sanitize(
    contactInfo.heroHeadline || "CRIAMOS O <br /><span class='text-orion-glow'>INESQUECÍVEL</span>"
  );
  ...
  return (
    <section className="relative min-h-[90vh] md:min-h-screen flex items-center justify-center overflow-hidden px-4">
      ...
      <div className="absolute bottom-8 left-1/2 -translate-x-1/2 animate-bounce opacity-30 hidden md:block">
        <ArrowDown className="w-5 h-5 text-orion-accent" aria-hidden="true" />
      </div>
    </section>
  );
};
