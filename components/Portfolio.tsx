...
import { ChevronRight, X, Layers, ChevronLeft } from 'lucide-react';
...
const Portfolio: React.FC = () => {
  ...
  useEffect(() => {
    ...
    else { document.body.style.overflow = 'unset'; setCurrentImg(0); }
  }, [selectedItem]);
    ...
    <section id="collection" className="py-20 relative z-10 px-4 md:px-0">
      <div className="container mx-auto px-2">
        <div className="flex flex-col md:flex-row justify-between items-start md:items-end mb-12 gap-6">
          ...
          <div className="flex flex-wrap gap-2 md:gap-3">
            {categories.map(c => (
              <button key={c} onClick={() => setActiveCategory(c)} className={`px-3 py-1.5 text-[9px] md:text-xs uppercase tracking-widest border transition-all ${activeCategory === c ? 'border-orion-accent text-white bg-orion-purple/20' : 'border-white/10 text-gray-500'}`}>{c}</button>
            ))}
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
          {items.map(item => (
            <div key={item.id} onClick={() => setSelectedItem(item)} className="group relative aspect-[4/5] bg-orion-dark border border-white/5 overflow-hidden rounded-sm cursor-pointer">
              <img src={item.imageUrl} alt={item.name} className="absolute inset-0 w-full h-full object-cover group-hover:scale-110 transition-transform duration-700" />
              ...
              <div className="absolute bottom-0 left-0 p-6 w-full">
                ...
                <div className="flex items-center gap-2 text-[10px] text-gray-400 mt-2 uppercase tracking-widest">Detalhes <ChevronRight size={12}/></div>
              </div>
            </div>
          ))}
        </div>
      ...
      {selectedItem && createPortal(
        <div className="fixed inset-0 z-[100] flex items-center justify-center">
          <div className="absolute inset-0 bg-orion-black/95 backdrop-blur-xl" onClick={() => setSelectedItem(null)}></div>
          ...
          <div className="relative w-full h-full md:h-auto md:max-w-5xl md:aspect-[16/9] bg-orion-dark flex flex-col md:flex-row overflow-hidden md:rounded-lg border-y md:border border-white/10">
            <button onClick={() => setSelectedItem(null)} className="absolute top-4 right-4 z-[110] p-2 bg-black/40 rounded-full text-white"><X size={24}/></button>
            
            <div className="flex-1 relative bg-black flex items-center justify-center">
              <img src={images[currentImg]} className="w-full h-full object-contain md:object-cover" />
              {images.length > 1 && (
                <div className="absolute inset-x-0 bottom-4 flex justify-center gap-2 z-20">
                  {images.map((_, i) => <div key={i} className={`w-1.5 h-1.5 rounded-full ${i === currentImg ? 'bg-orion-accent' : 'bg-white/30'}`} />)}
                </div>
              )}
              {images.length > 1 && (
                <>
                  <button onClick={(e) => {e.stopPropagation(); setCurrentImg((currentImg - 1 + images.length) % images.length)}} className="absolute left-2 top-1/2 -translate-y-1/2 p-2 bg-black/40 rounded-full"><ChevronLeft/></button>
                  <button onClick={(e) => {e.stopPropagation(); setCurrentImg((currentImg + 1) % images.length)}} className="absolute right-2 top-1/2 -translate-y-1/2 p-2 bg-black/40 rounded-full"><ChevronRight/></button>
                </>
              )}
            </div>
            ...
            <div className="w-full md:w-[400px] p-8 flex flex-col bg-orion-dark/80 backdrop-blur-md overflow-y-auto custom-scrollbar">
              ...
              <div className="space-y-3 mb-8">
                {selectedItem.specs.map((s, i) => (
                  <div key={i} className="text-xs text-gray-300 flex items-center gap-2"><div className="w-1 h-1 bg-orion-purple rounded-full"></div>{s}</div>
                ))}
              </div>

              <button 
                onClick={() => window.open(`https://wa.me/${contactInfo.whatsapp}?text=Interesse em ${selectedItem.name}`, '_blank')}
